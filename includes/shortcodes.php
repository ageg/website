...
function register_button($buttons) {
   array_push($buttons, "heading","button", "checklist","highlight","tooltip","separator","accordion","tabs","vtabs","big_title","feat_text","testimonials","separator","posts_carousel","portfolio_carousel","slider","table","price_table","message","person","icon","border","bar_graphs","counter_circles","text_box","separator","column","column_row","separator","youtube","vimeo","separator");    
   return $buttons;
}
...
function add_plugin($plugin_array) {
   ...
   $plugin_array['vtabs'] = get_template_directory_uri().'/js/customcodes.js';
...
// Vtabs
add_shortcode('vtabs', 'shortcode_vtabs');
function shortcode_vtabs( $atts, $content = null ) {
...
function shortcode_vtab( $atts, $content = null ) {
	...
	return '<div class="vtab-content"  id="' . $atts['id'] . '">' . prefix_remove_wpautop(do_shortcode($content)) . '</div>';	
	...
	$str='
			<div class="four columns">
				<div class="member pic">
					<h4><a href="'.$atts["email"].'">'.$atts["title"].'</a></h4>
					'.($atts["picture_url"] ? '<img src="'.$atts["picture_url"].'">' : '<img src="'. get_template_directory_uri() .'/images/user.png">').'
					<p>'.$atts["name"].'<br><small>'.$atts["details"].'</small></p></p>
				</div>
			</div>';
